...
$MsgBoxStr ="EJN V18 sep 2023  Copy Files to B"
$MaxToCopyFiles = 1500                                                                                    ;Let's not copy more than 1500 files
$try=0
    MsgBox($MB_TASKMODAL, $MsgBoxStr, "Started waiting for B:",2)
Do
	Sleep(999)
    $try = $try + 1
    If $try = 2 Then 
		MsgBox($MB_TASKMODAL, $MsgBoxStr, "Backup drive not ready",3) 
		 Exit
	EndIf	 
Until FileExists("b:\")
_CopyFolderNoOverwrite("c:\Users\ednie\Documents\Files\" , "b:\DataEd\My Documents\Files\")    ; kopieer en overschrijf oudere bestanden
...
Func _CopyFolderNoOverwrite($SourceFolder, $DestinationFolder)
    ...
    If Not IsArray($Array) Or Not $Array[1] Then Return                                            ;MsgBox(4096, 'No Files Found', 'Cannot Find any Files in ' & $SourceFolder,3)
    ...
    Next
	If $CopyArray[0][0]=0 Then Return  MsgBox(4096, 'No Files to Copy Found', 'Cannot Find any Files to Copy in ' & $SourceFolder,3)
