...
$MsgBoxStr ="EJN V04 Apr 2025   iPhonePics Copy Files to A"
$MaxToCopyFiles = 1000       



;Let's not copy more than 1000 files
$try=0
Do
 		If $Nodisplay = False Then MsgBox(0, $MsgBoxStr, "Started waiting for A:",1)
	Sleep(99)
    $try = $try + 1
    If $try = 200 Then ExitLoop
	Until FileExists("a:\")                                                                  ; Make sure the network drive is ready

 $FolderToCopyTo = "a:\DataEd\Fotos\Fotos-IPhone\" & @YEAR & "\"
 if NOT FileExists($FolderToCopyTo) Then DirCreate($FolderToCopyTo)
_CopyFolderNoOverwrite("c:\Users\ednie\Pictures\iCloud Photos\Photos\" , $FolderToCopyTo)    ; kopieer en overschrijf oudere bestanden

 $FolderToCopyTo = "c:\Users\ednie\Documents\Files\Foto-Iphone\"
 if NOT FileExists($FolderToCopyTo) Then DirCreate($FolderToCopyTo)
_CopyFolderNoOverwrite("c:\Users\ednie\Pictures\iCloud Photos\Photos\" , $FolderToCopyTo)    ; kopieer en overschrijf oudere bestanden
		If $Nodisplay = False Then MsgBox(0, $MsgBoxStr, "Copy Ended",4)
