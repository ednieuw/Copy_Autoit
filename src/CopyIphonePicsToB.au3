...
; Dit programma kopieert fotobestanden van de Iphone Icloud folder naar de backup drive en de documenten-schijf op C:
; Als het huidige systeemjaar als folder nog niet bestaat maakt deze de folder aan zodat de foto's per jaar gegroepeerd worden.
...
$MsgBoxStr ="EJN V19 sep 2023  iPhonePicsCopy Copy Files to B:"
$try=0
Do
    MsgBox(0,$MsgBoxStr, "Started waiting for B:",1)
	Sleep(999)
    $try = $try + 1
    If $try = 2 Then ExitLoop
	Until FileExists("b:\")                                                                  ; Make sure the network drive is ready

 $FolderToCopyTo = "b:\DataEd\Fotos\Fotos-IPhone\" & @YEAR & "\"
 if NOT FileExists($FolderToCopyTo) Then DirCreate($FolderToCopyTo)
_CopyFolderNoOverwrite("c:\Users\ednie\iCloudPhotos\Photos\" , $FolderToCopyTo)    ; kopieer en overschrijf oudere bestanden

 $FolderToCopyTo = "c:\Users\ednie\Documents\Files\Foto-Iphone\"
 if NOT FileExists($FolderToCopyTo) Then DirCreate($FolderToCopyTo)
_CopyFolderNoOverwrite("c:\Users\ednie\iCloudPhotos\Photos\" , $FolderToCopyTo)    ; kopieer en overschrijf oudere bestanden
MsgBox(0, $MsgBoxStr, "Copy Ended",4)

Func _CopyFolderNoOverwrite($SourceFolder, $DestinationFolder)                                ; if year is "" then all files are copied
	...
	$CopyArray[0][0]=0
	    For $i = 1 To $Array[0]
		   if StringLeft(FileGetTime($TCopyArray[$i][0],1,1) ,4) = @YEAR Then                    ; "2022" Then
	...
	; Create GUI
	GUICreate("EJN V15 sep 2022  Fotos-IPhone to B:", 800, 300)
	$hEdit = GUICtrlCreateEdit("Copied files" & @CRLF, 2, 2, 794, 268)
	_GUICtrlEdit_SetLimitText($hEdit, 300000)                                                     ;300000 chars in messqagebox instead of default 30000. $hEdit,, -1) is unlimited size of the message box
	GUISetState()
